...
# filter out of business results
cfs_clean_oob <- cfs_clean %>%
  filter(results == "Out of Business")

# plot for out of business inspections
ggplot(cfs_clean_oob, aes(x = longitude, y = latitude)) +
  geom_bin2d(bins = 30, alpha = 0.8) +  
  scale_fill_gradient(low = "yellow", high = "blue") +
  labs(title = "Geographic Distribution of Out of Business Inspections",
       x = "Longitude",
...
cfs_clean_pass <- cfs_clean %>%
  filter(results == "Pass")

# plot for passing inspections
ggplot(cfs_clean_pass, aes(x = longitude, y = latitude)) +
  geom_bin2d(bins = 30, alpha = 0.8) +  
  scale_fill_gradient(low = "yellow", high = "blue") +
  labs(title = "Geographic Distribution of Passed Inspections",
       x = "Longitude",
       y = "Latitude",
       fill = "Density") +
  theme_minimal()
