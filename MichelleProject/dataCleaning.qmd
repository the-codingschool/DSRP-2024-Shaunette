...
total_coords <- nrow(coords)

# Function to print progress
print_progress <- function(current, total) {
  percent_complete <- round((current / total) * 100, 1)
  coords_left <- total - current
  cat(sprintf("\rProcessing: %.1f%% complete | Coordinates left: %d", percent_complete, coords_left))
}

# Initialize an empty data frame with the expected columns
exact_locs <- data.frame(
  latitude = numeric(),
  longitude = numeric(),
  address_found = character(),
  stringsAsFactors = FALSE
)

# Process each coordinate with a loop and print progress
for (i in 1:total_coords) {
  single_coord <- coords[i, , drop = FALSE] # Create a single-row data frame
  
  start_time <- Sys.time()
  loc <- reverse_geocode(.tbl = single_coord, lat = latitude, long = longitude, method = 'osm', full_results = TRUE)
  end_time <- Sys.time()
  
  time_taken <- as.numeric(difftime(end_time, start_time, units = "secs"))
  
  # Ensure loc has the same structure as exact_locs before binding
  if (all(names(loc) %in% names(exact_locs))) {
    exact_locs <- rbind(exact_locs, loc)
  }
  
  # Print progress
  print_progress(i, total_coords)
}
...
```{r}
#to see column names available
colnames(exact_locs)

#municipality seems to be a good way of grouping places:
filtered_locs <- exact_locs %>%
  select(latitude, longitude, quarter, municipality, city, postcode)

saveRDS(filtered_locs, file = "/Users/michelle/Downloads/DSRP/DSRP-2024-Shaunette/MichelleProject/filtered_locs.rds")
```
